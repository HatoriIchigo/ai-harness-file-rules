...
- `class.extend` は継承先の**ファイルパス**（プロジェクトルート相対、または絶対パス）を指定する。継承すべきクラス名はファイル名からの推測ではなく、そのファイル自体を AST 解析して宣言されているクラス名を取得する（同名慣習に依存しない）。判定は単純名（パッケージ／名前空間修飾を除いた末尾セグメント）の一致で、`extends` 節のみ見る（`implements` は対象外）。C++ / Python の多重継承はいずれかの基底が一致すれば可。パス解決には hook の `cwd` を使うため、`cwd` が取得できない場合はこの検査を含む当該ファイルの検査全体をスキップ（許可）する。指定ファイルが存在しない・未対応言語・クラス宣言が無い等で解決できない場合は deny（フェイルクローズ）。
